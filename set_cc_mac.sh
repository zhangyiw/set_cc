...
# set_cc_mac.sh - 切换不同AI模型环境变量的脚本（兼容旧版本Bash）
...
switch_model() {
    ...
    case $model in
        kimi|glm|qwen|deepseek)
            ...
            # 检查并更新 ~/.zshrc
            if grep -q "export ANTHROPIC_BASE_URL=" ~/.zshrc 2>/dev/null; then
                # 如果已存在ANTHROPIC_BASE_URL，则替换
                #sed -i '' "s/export ANTHROPIC_BASE_URL=.*/export ANTHROPIC_BASE_URL=\"$(get_model_config $model base)\"/" ~/.zshrc
                sed -i '' "s#export ANTHROPIC_BASE_URL=.*#export ANTHROPIC_BASE_URL=\"$(get_model_config $model base)\"#" ~/.zshrc
            else
                # 如果不存在，则追加
                echo "export ANTHROPIC_BASE_URL=\"$(get_model_config $model base)\"" >> ~/.zshrc
            fi
            
            if grep -q "export ANTHROPIC_MODEL=" ~/.zshrc 2>/dev/null; then
                # 如果已存在ANTHROPIC_MODEL，则替换
                #sed -i '' "s/export ANTHROPIC_MODEL=.*/export ANTHROPIC_MODEL=\"$(get_model_config $model model)\"/" ~/.zshrc
                sed -i '' "s#export ANTHROPIC_MODEL=.*#export ANTHROPIC_MODEL=\"$(get_model_config $model model)\"#" ~/.zshrc
            else
                # 如果不存在，则追加
                echo "export ANTHROPIC_MODEL=\"$(get_model_config $model model)\"" >> ~/.zshrc
            fi
            
            if grep -q "export ANTHROPIC_AUTH_TOKEN=" ~/.zshrc 2>/dev/null; then
                # 如果已存在ANTHROPIC_AUTH_TOKEN，则替换
                #sed -i '' "s/export ANTHROPIC_AUTH_TOKEN=.*/export ANTHROPIC_AUTH_TOKEN=\"$(get_model_config $model key)\"/" ~/.zshrc
                sed -i '' "s#export ANTHROPIC_AUTH_TOKEN=.*#export ANTHROPIC_AUTH_TOKEN=\"$(get_model_config $model key)\"#" ~/.zshrc
            else
                # 如果不存在，则追加
                echo "export ANTHROPIC_AUTH_TOKEN=\"$(get_model_config $model key)\"" >> ~/.zshrc
            fi

            echo "✅ 已切换到 $model 模型，请执行 'source ~/.zshrc' 或重新打开终端使更改永久生效."
            ;;
        *)
            echo "❌ 错误: 未知模型 '$model'"
            show_usage
